...
class TrafficSignConfig:
    def __init__(self, auto_detect: bool = True):
        ...
        # Các biển báo có viền đỏ nhưng tâm xanh (dễ bị nhầm mask)
        self.PROBLEM_SIGNS = {
            "cam_do_xe", 
            "cam_dung_do_xe"
        }
        # Tỷ lệ phóng to BBox (ví dụ: 1.3 = tăng 30%)
        self.BBOX_EXPAND_SCALE = 1.3 
        # Ngưỡng IoU để coi 2 BBox là cùng 1 biển báo
...
def expand_bbox(bbox: Tuple, scale: float, frame_shape: Tuple) -> Tuple:
    ...
    # Tính toán lại w, h sau khi đã kẹp x, y
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            while True:
                ...
                # ==========================================================
                # === BẮT ĐẦU LOGIC MỚI: XỬ LÝ BIỂN BÁO "PROBLEM SIGNS" ===
                # ==========================================================
                ...
                # --- Bước 1: Phân loại các phát hiện ---
                for (bbox, color, metrics) in recognized_detections:
                    sign_name = metrics.get('sign_name')
                    
                    if sign_name in config.PROBLEM_SIGNS:
                        if color == 'red':
                            # Đây là biển "vấn đề" được phát hiện bởi mask đỏ (ưu tiên)
                            final_detections.append((bbox, color, metrics))
                            red_problem_bboxes.append(bbox)
                        elif color == 'blue':
                            # Đây là biển "vấn đề" bị phát hiện bởi mask xanh (cần kiểm tra)
                            blue_problem_detections.append((bbox, color, metrics))
                    else:
                        # Đây là các biển báo thông thường, thêm vào danh sách cuối cùng
                        final_detections.append((bbox, color, metrics))
                
                # --- Bước 2: Xử lý các phát hiện "blue problem" ---
                if blue_problem_detections:
                    for (blue_bbox, blue_color, blue_metrics) in blue_problem_detections:
                        is_overlapped_by_red = False
                        
                        # Kiểm tra xem nó có trùng với một phát hiện "red" nào không
                        for red_bbox in red_problem_bboxes:
                            ...
                            if iou > config.BBOX_OVERLAP_IOU:
                                ...
                                break # Đã tìm thấy 1 BBox đỏ trùng, bỏ qua BBox xanh này
                        
                        if is_overlapped_by_red:
                            # Bỏ qua, vì chúng ta ưu tiên BBox "red" đã được thêm vào
                            pass
                        else:
                            # Đây là phát hiện "blue-only" (ở xa)
                            # Mở rộng BBox của nó để bao trọn viền đỏ
                            ...
                            # Thêm BBox đã mở rộng vào danh sách cuối cùng
                ...
                # Cập nhật tổng số biển báo dựa trên danh sách cuối cùng
                total_signs_recognized += len(final_detections)

                # Draw on frame (SỬ DỤNG 'final_detections' THAY VÌ 'recognized_detections')
                frame_output = visualizer.draw_all(frame, frame_num,
                                                 final_detections, roi_pixel_map)
                video_writer.write(frame_output)

                if final_detections: # <-- Thay đổi ở đây
