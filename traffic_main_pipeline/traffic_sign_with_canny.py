...
"""
TRAFFIC SIGN DETECTION SYSTEM - GIẢI PHÁP 4 (CANNY EDGE DETECTION)

LOGIC ĐÃ THAY ĐỔI:
====================
- Bỏ hoàn toàn lọc màu HSV/YCrCb.
- PASS 1: Detect (Canny Edge + Shape) -> Đoán màu (YCrCb-Mean) -> Verify (SVM) -> Track/Smooth
- PASS 2: Recognize (SVM) -> Render

ƯU ĐIỂM:
- KHÔNG CẦN TUNE MÀU.
- Cực kỳ ổn định với ánh sáng, chạy được cả ban đêm (nếu có đèn chiếu).
...
class TrafficSignConfig:
    def __init__(self, auto_detect: bool = True):
        ...
        self.MASK_VIDEO_CANNY = DRIVE_PATH + 'videos/mask_video_canny.mp4'
        # (Xóa mask blue/red/yellow)
        ...
        # Đã xóa COLOR_PARAMS, CLAHE, SATURATION
        self.CANNY_BLUR_KSIZE = 5     # Kích thước Gaussian Blur
        self.CANNY_THRESHOLD_1 = 50   # Ngưỡng dưới
        self.CANNY_THRESHOLD_2 = 150  # Ngưỡng trên
        self.CANNY_MORPH_KSIZE = 3    # Kích thước kernel để nối cạnh
        self.CANNY_MORPH_ITER = 2     # Số lần DILATE
        
        # --- (MỚI) CẤU HÌNH ĐOÁN MÀU (YCrCb-MEAN) ---
        # Ngưỡng này dùng để đoán màu 'red', 'blue', 'yellow'
        # Cr > 135 là Đỏ
        self.COLOR_GUESS_CR_MIN = 135 
        # Cb > 135 là Xanh
        self.COLOR_GUESS_CB_MIN = 135 
        # Cr < 130 và Cb < 120 là Vàng
...
# =============================================================================
# CLASS 3: DETECTOR (LOGIC CANNY MỚI)
# =============================================================================
class TrafficSignDetector:
    ...
    def _preprocess_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Chúng ta blur frame gốc trước khi chuyển đổi
    ...
    def _get_dominant_color(self, roi_ycrcb: np.ndarray) -> str:
        ...
        """
        try:
            # Tính giá trị trung bình của kênh Cr và Cb
            mean_vals = cv2.mean(roi_ycrcb)
            mean_cr = mean_vals[1]
            mean_cb = mean_vals[2]
            
            # Kiểm tra logic đoán màu
            if mean_cr > self.config.COLOR_GUESS_CR_MIN:
                return 'red'
            if mean_cb > self.config.COLOR_GUESS_CB_MIN:
                return 'blue'
    ...
    def process_frame(self, frame: np.ndarray, full_frame_dims: Tuple) -> Tuple[List[Tuple], Dict[str, np.ndarray]]:
        ...
        for contour in contours:
            ...
            if (area >= circle_cfg['min_area'] and area <= circle_cfg['max_area']):
                ...
                if is_valid:
                    ...
                    continue # Đã tìm thấy, không cần check tam giác
        ...
        # Trả về mask rỗng cho các màu cũ (để ghi video mask nếu bật)
        masks_dict['blue'] = np.zeros((h_full, w_full), dtype=np.uint8)
        masks_dict['red'] = np.zeros((h_full, w_full), dtype=np.uint8)
        masks_dict['yellow'] = np.zeros((h_full, w_full), dtype=np.uint8)
...
def process_frame_worker(args):
    """
    Worker function cho Giai đoạn 1: Detect (Canny) + Verify (SVM)
    """
...
# =============================================================================
class Visualizer:
    # (Không thay đổi - Giữ nguyên class Visualizer)
    def __init__(self, config: TrafficSignConfig):
        self.config = config
        # (Sửa lỗi: xóa self.roi_colors vì không còn dùng)
    ...
    def draw_all(self, frame: np.ndarray, frame_num: int,
        ...
        if self.config.DEBUG_MODE:
            ...
            # (Xóa _draw_roi_boxes vì không còn ROI)
...
# =============================================================================
# (Giữ nguyên) ParallelFrameReader, SignDetectorSVM, SignRecognizerSVM
# =============================================================================
...
class SignDetectorSVM:
    # (Không thay đổi - Đây là model HOG Xám)
...
def convert_roi_to_pixels(roi_percent: Tuple, w_full: int, h_full: int) -> Tuple:
    # (Không dùng nữa nhưng giữ lại phòng sau này)
...
def is_bbox_in_roi(bbox: Tuple, roi_params: Tuple, overlap_threshold: float = 0.5) -> bool:
    ...
    x_max, y_max = x + w, y + h
    inter_x1 = max(x, roi_x_start); inter_y1 = max(y, roi_y_start)
    inter_x2 = min(x_max, roi_x_end); inter_y2 = min(y_max, roi_y_end)
...
# =============================================================================
# MAIN FUNCTION (LOGIC CANNY - CHỐNG FLICKER)
# =============================================================================
def main():
    print("=" * 70)
    print("  TRAFFIC SIGN DETECTION (CANNY EDGE + ANTI-FLICKER)")
    print("=" * 70)
    ...
    try:
        ...
        # (Xóa ROI Pixel Map vì không còn dùng)
        roi_pixel_map = {} 

        # =====================================================================
        # PASS 1: PARALLEL DETECT (Canny) + VERIFY (SVM) + TRACK
        # =====================================================================
        print("\n" + "=" * 70)
        print("⚡ PASS 1: PARALLEL DETECT (Canny) + VERIFY (SVM) + TRACK")
        print("=" * 70)
        ...
        if config.SAVE_MASK_VIDEOS:
            ...
            # Chỉ ghi 1 mask Canny
            canny_writer = cv2.VideoWriter(config.MASK_VIDEO_CANNY, fourcc, fps, (w_crop, h_crop), False)
        ...
        # Truyền đường dẫn model HOG Xám
            ...
            while True:
                ...
                if len(frame_batch) >= config.BATCH_SIZE:
                    ...
                    for frame_num, detections, masks in results:
                        ...
                        if config.SAVE_MASK_VIDEOS and canny_writer and 'canny' in masks:
                            canny_writer.write(masks['canny'])
        ...
        frame_reader.stop()
        if canny_writer: canny_writer.release()
        if debug_writer:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            while True:
                ...
                if smoothed_candidates:
                    ...
                    # (MỚI) Lưu ảnh tại đây (sau khi recognize)
                    if config.SAVE_DETECTOR_IMAGES:
                ...
                # 4. (GIỮ NGUYÊN) Logic "Problem Signs"
                final_detections = []
        ...
        print("\n" + "=" * 70)
        print("✅ PROCESSING COMPLETE (CANNY LOGIC)!")
        print("=" * 70)
        ...
        if config.SAVE_MASK_VIDEOS: print(f"📁 Canny Mask: {config.MASK_VIDEO_CANNY}")
        ...
        print(f"   ✅ CANNY EDGE PIPELINE (No color tuning)")
