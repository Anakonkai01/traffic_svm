...
"""
TRAFFIC SIGN DETECTION SYSTEM - GIẢI PHÁP 3 (CHỐNG FLICKER)

LOGIC ĐÃ THAY ĐỔI:
====================
- PASS 1: Detect (Color+Shape) -> Verify (SVM) -> Track/Smooth
- PASS 2: Recognize (SVM) -> Render

ƯU ĐIỂM:
- Triệt tiêu "existence flicker" (nhấp nháy Tắt/Bật)
- Video output sẽ cực kỳ mượt mà.

NHƯỢC ĐIỂM:
- Pass 1 sẽ chậm hơn một chút vì worker phải chạy SVM.
- Pass 2 sẽ nhanh hơn rất nhiều.
"""


DRIVE_PATH = "../"

# SỬ DỤNG MODEL HOG XÁM GỐC CỦA BẠN
...
class TrafficSignConfig:
    def __init__(self, auto_detect: bool = True):
        ...
        self.TRACKING_PARAMS = {
            'max_gap_sec': 1.0, # Thử tăng lên 2.0 nếu vẫn flicker
            'iou_threshold': 0.1,
            'smoothing_window': 15
        }

        # --- LOGIC ĐIỀU CHỈNH BBOX SAU NHẬN DIỆN ---
        self.PROBLEM_SIGNS = {
            "cam_do_xe", 
            "cam_dung_do_xe"
        }
...
class SignTracker:
            ...
            if 1 < frame_gap <= self.max_gap_frames:
                ...
                for j in range(1, frame_gap):
                    ...
                    interpolated.append({
                        ...
                        'interpolated': True # Đánh dấu đây là frame nội suy
                    })
    ...
    def build_smoothed_cache(self):
        ...
        for track_id, track_data in self.tracks.items():
            ...
            # 1. Lấp đầy các frame bị SVM loại bỏ
...
# =============================================================================
# PARALLEL FRAME PROCESSOR (LOGIC MỚI - CHỐNG FLICKER)
# =============================================================================

# KHỞI TẠO BIẾN TOÀN CỤC CHO WORKER
# Dùng để lưu trữ các model, tránh load lại mỗi lần gọi hàm
worker_models = {}

def initialize_worker(detector_model_path):
    """
    Hàm này được gọi một lần khi worker process khởi động.
    Nó tải các model vào một biến 'global' của riêng worker đó.
    """
    global worker_models
    if not worker_models:
        print(f"Worker (PID {os.getpid()}) initializing models...")
        
        # SỬ DỤNG MODEL HOG XÁM (SignDetectorSVM)
        worker_models['detector_svm'] = SignDetectorSVM(detector_model_path)
        
        worker_models['config'] = None

def process_frame_worker(args):
    """
    Worker function cho Giai đoạn 1: Detect + Verify
    """
    frame_num, frame, config, full_frame_dims = args
    global worker_models

    # Lấy config
    if worker_models['config'] is None:
        worker_models['config'] = config

    # Lấy detector (Color+Shape)
    detector = TrafficSignDetector(worker_models['config'])
    
    # Lấy detector_svm (từ biến global của worker)
    detector_svm = worker_models.get('detector_svm')
    if detector_svm is None:
        print(f"Worker (PID {os.getpid()}) LỖI: Model chưa được khởi tạo.")
        return frame_num, [], {}

    # 1. Detect (Color+Shape) -> Lấy ứng cử viên thô
    raw_detections, masks = detector.process_frame(frame, full_frame_dims)

    # 2. Verify (SVM) -> Lọc ứng cử viên
    verified_detections = []
    if raw_detections:
    ...
    # 3. Trả về các ứng cử viên ĐÃ ĐƯỢC XÁC THỰC (nhưng bị flicker)
...
class Visualizer:
    ...
    def draw_debug_detections(self, frame: np.ndarray, frame_num: int,
        ...
        LƯU Ý: 'detections' BÂY GIỜ LÀ CÁC PHÁT HIỆN ĐÃ ĐƯỢC VERIFY.
        """
        frame_output = frame.copy()
        ...
        # Sửa lại tiêu đề: "Post-SVM Detections"
...
# =============================================================================
# CLASS 5.1: SVM DETECTOR (HOG MÀU - BẠN CÓ THỂ GIỮ NẾU MUỐN DÙNG SAU)
# =============================================================================
class SignDetectorSVM_ColorHOG:
    def __init__(self, model_path: str):
        print(f"Loading SVM Detector (COLOR HOG) from: {model_path}")
        try:
            self.svm = cv2.ml.SVM_load(model_path)
            if self.svm is None:
                raise Exception(f"Cannot load model from {model_path}")
        ...
        self.hog = cv2.HOGDescriptor(winSize, blockSize, blockStride, cellSize, nbins)
        self.resize_dim = RESIZE_DIM
        print("✓ SVM Detector (COLOR HOG) loaded successfully.")
    ...
    def verify(self, frame: np.ndarray, bbox: Tuple) -> bool:
        ...
        try:
            sign_roi = frame[y:y+h, x:x+w]
            sign_resized = cv2.resize(sign_roi, self.resize_dim, interpolation=cv2.INTER_AREA)

            b_channel, g_channel, r_channel = cv2.split(sign_resized)
            ...
            _, result = self.svm.predict(final_features.reshape(1, -1))
            label = int(result[0][0])
            return label == 1
        except:
            return False
...
def calculate_iou(box1: Tuple, box2: Tuple) -> float:
    """Tính Intersection over Union (IoU) giữa 2 BBox."""
    x1, y1, w1, h1 = box1
    x2, y2, w2, h2 = box2
    x1_max, y1_max = x1 + w1, y1 + h1
    x2_max, y2_max = x2 + w2, y2 + h2
...
def expand_bbox(bbox: Tuple, scale: float, frame_shape: Tuple) -> Tuple:
    """Mở rộng BBox từ tâm của nó và giới hạn trong khung hình."""
    x, y, w, h = bbox
...
# =============================================================================
# MAIN FUNCTION (LOGIC MỚI - CHỐNG FLICKER)
# =============================================================================
def main():
    print("=" * 70)
    print("  TRAFFIC SIGN DETECTION (ANTI-FLICKER LOGIC)")
    print("=" * 70)
    ...
    try:
        ...
        # KHÔNG cần khởi tạo 'detector_svm' ở đây nữa,
        # vì nó sẽ được khởi tạo trong các worker
        ...
        # Initialize SVM Recognizer (vẫn cần cho Pass 2)
        try:
        ...
        # =====================================================================
        # PASS 1: PARALLEL DETECT + VERIFY + TRACK
        # =====================================================================
        print("\n" + "=" * 70)
        print("⚡ PASS 1: PARALLEL DETECT + VERIFY + TRACK")
        print("=" * 70)
        ...
        # Chuẩn bị đối số cho hàm khởi tạo worker
        # Chúng ta truyền đường dẫn HOG XÁM
        init_args = (MODEL_DETECTOR_PATH,)
        
        # Sửa đổi Pool: Thêm 'initializer' và 'initargs'
        with Pool(processes=config.NUM_WORKERS, 
            ...
            while True:
                ...
                # KHÔNG cần truyền đường dẫn model vào batch nữa
                ...
                if len(frame_batch) >= config.BATCH_SIZE:
                    # Chạy process_frame_worker (Detect + Verify)
                    ...
                    for frame_num, detections, masks in results:
                        # 'detections' BÂY GIỜ LÀ ĐÃ ĐƯỢC VERIFY
                        if detections:
                    ...
                    if debug_writer and frame_full_batch:
                        for frame_num, frame_full in frame_full_batch:
                            # 'detections_map' chứa các BBox đã verify
        ...
        # Build smoothed cache (BƯỚC CHỐNG FLICKER QUAN TRỌNG)
        ...
        saved_images_count = 0 # (Việc lưu ảnh ở đây sẽ bị loại bỏ)
        
        if config.SAVE_DETECTOR_IMAGES:
            print("\n⚠️  Lưu ý: Logic SAVE_DETECTOR_IMAGES đã bị vô hiệu hóa")
            print("   do 'verify' đã chuyển sang Pass 1.")
            config.SAVE_DETECTOR_IMAGES = False
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            while True:
                ...
                # 1. Get smoothed detections (ĐÃ SẠCH VÀ MƯỢT)
                smoothed_candidates = tracker.get_smoothed_detections(frame_num)

                # 2. Verify with SVM Detector (ĐÃ XÓA BỎ)
                # (Không còn 'verified_detections' nữa)
                
                # 3. Recognize with SVM Recognizer
                ...
                # ==========================================================
                # === LOGIC XỬ LÝ "PROBLEM SIGNS" (VẪN GIỮ NGUYÊN) ===
                # ==========================================================
                ...
                frame_output = visualizer.draw_all(frame, frame_num,
                                                 final_detections, roi_pixel_map)
        ...
        if config.SAVE_DEBUG_VIDEO:
            print(f"📁 Debug (Post-SVM): {config.DEBUG_VIDEO_PATH}")
        ...
        print(f"   ✅ ANTI-FLICKER LOGIC APPLIED")
